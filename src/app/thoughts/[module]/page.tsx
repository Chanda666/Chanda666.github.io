...
function loadModuleData(slug: string, locale?: string): ModulePageData | null {
  const cfg = getTomlContent<ThoughtsPageConfig>('thoughts.toml', locale);
  if (!cfg) return null;
...
export function generateStaticParams() {
  const cfg = getTomlContent<ThoughtsPageConfig>('thoughts.toml');
  if (!cfg) return [];
...
export default async function ThoughtsModulePage({
  params,
}: {
  params: Promise<{ module: string }>;
}) {
  ...
  return <ThoughtsModuleDetailPageClient dataByLocale={dataByLocale} defaultLocale={runtimeI18n.defaultLocale} />;
}